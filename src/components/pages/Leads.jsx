...
import { companyService, contactService, leadService } from "@/services";
...
return (
    ...
    <div
        ...
        <div>
            ...
            <p className="text-surface-600 mt-1">Manage your sales pipeline ({filteredLeads.length}leads)
                          </p>
        </div>
        <div className="flex items-center gap-4">
            <div
                className="text-sm text-surface-600 bg-white px-4 py-2 rounded-lg border border-surface-200">Total Value: ${leads.reduce((sum, lead) => sum + (lead.value || 0), 0).toLocaleString()}
            </div>
            <Button variant="primary" onClick={handleCreateLead} icon="Plus">Create New Lead
                          </Button>
        </div>
    </div>
    {/* Search */}
    ...
    {filteredLeads.length === 0 ? <EmptyState
        ...
        icon="TrendingUp" /> : <div className="bg-white rounded-xl shadow-sm border border-surface-200 p-6">
                ...
                return (
                    <div
                        ...
                        <div className="space-y-3 max-h-96 overflow-y-auto">
                            {stageLeads.map((lead, index) => <motion.div
                                key={lead.Id}
                                initial={{
                                    opacity: 0,
                                    y: 20
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-start justify-between">
                                        <h4
                                            className="font-medium text-surface-900 text-sm truncate flex-1 cursor-pointer hover:text-primary-600 transition-colors"
                                        ...
                                        <div className="flex items-center gap-1">
                                            <button
                                                onClick={() => handleEditLead(lead)}
                                                className="opacity-0 group-hover:opacity-100 p-1 text-surface-400 hover:text-primary-600 transition-all">
                                            ...
                                            <button
                                                ...
                                                <div className="flex items-center justify-between">
                                                    <span className="text-sm font-medium text-accent-600">${lead.value?.toLocaleString() || "0"}
                                                    </span>
                                                ...
                                                {lead.expectedCloseDate && <div className="text-xs text-surface-500">
                                                    <ApperIcon name="Calendar" className="w-3 h-3 inline mr-1" />
                                                    {format(new Date(lead.expectedCloseDate), "MMM d, yyyy")}
                                                </div>}
                                            </button></div>
                                    </div></div></motion.div>)}
                            {stageLeads.length === 0 && <div className="text-center py-8 text-surface-400">
